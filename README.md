...
**Optional: split secrets** — set these to use **different** Bearer values on each surface (when unset, routes fall back to `HEY_ADMIN_TOKEN` / `HEY_ADMIN_TOKEN_PREVIOUS`, so a single password still works):

| Env | Protects |
| --- | --- |
| `HEY_ADMIN_TOKEN` (+ optional `HEY_ADMIN_TOKEN_PREVIOUS`) | Main SPA lock, `PUT/DELETE` messages, `PUT /api/config`, backend log tail (`GET /api/admin/logs`). |
...
| `HEY_DOCS_ADMIN_TOKEN` (+ optional `…_PREVIOUS`) | `GET /api/openapi.yaml`, `/api/docs/` (Swagger UI), and `PUT /api/custom-head/clip/:label` (WAV upload via API). Use this when hitting the backend directly (e.g. `http://192.168.x.x:5100/api/docs`). |

`POST /api/auth/verify-admin` accepts optional `audience`: `main` (default), `training`, or `docs` so each UI can check the right secret before saving it client-side.
...
| `HEY_ADMIN_TOKEN` / `ADMIN_TOKEN` | **Backend env only**. Main SPA admin + messages/config/logs; optionally `HEY_TRAINING_ADMIN_TOKEN` / `HEY_DOCS_ADMIN_TOKEN` split training vs Swagger (see **Admin password**). |
| `HEY_TRAINING_ADMIN_TOKEN` | Optional. Training WAV API + `POST /api/custom-head/train`; falls back to main token when unset. |
| `HEY_DOCS_ADMIN_TOKEN` | Optional. OpenAPI + Swagger + `PUT /api/custom-head/clip`; falls back to main when unset. |
